...
class DataActivity extends SActivity {
  onCreate {
    contentView = new SVerticalLayout {
      ...
      STextView(getTomatoes(userId)).textSize(20 dip).<<.marginBottom(30 dip).>>
      STextView("Last Tomato").textSize(30 dip).<<.marginBottom(20 dip).>>
      STextView(getLastTomato(userId)).textSize(20 dip).<<.marginBottom(30 dip).>>
